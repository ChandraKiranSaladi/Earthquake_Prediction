...
#     take change values and divide by itself, then consider only those which come out to be non-zero
    ...
    # create windows such that we can calculate the above aggregations for rolling values of instances rather than keeping them fixed
    for windows in [10,100]:
        ...
        # Calculate the rolling features over standard deviation
        ...
        # Calculate the rolling features over mean
...
# Specify the file in which the submission content for kaggle is to be written.
submission = pd.read_csv('../input/sample_submission.csv', index_col='seg_id')
# Load test dataset
X_testset = pd.DataFrame(columns=X_trainset.columns, dtype=np.float64, index=submission.index)

# The test dataset is divided into 2624 files each representing a single segment of data
# Traverse across each of them for adding the features
for i, seg_id in enumerate(tqdm_notebook(X_testset.index)):
    ...
    # All of these features correspond to the ones specified for train dataset
